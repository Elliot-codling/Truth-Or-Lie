...
title = game_engine.properties_object("title", f"{file_dir}/textures/title_2d.png", w / 2 - (124 / 2 * scale), 30, 124 * scale, 15 * scale, False)
...
def init_buttons():         #initalises the buttons to be shown on screen
    ...
    global truth_button, lie_button
    truth_button = game_engine.properties_object("truth_button", f"{file_dir}/textures/truth_button_2d.png", 100, h - 200, 60 * scale, 15 * scale, False)
    lie_button = game_engine.properties_object("lie_button", f"{file_dir}/textures/lie_button_2d.png", w - (100 + 32 * scale), h - 200, 32 * scale, 15 * scale, False)
...
def main():             #main program that checks if the user has pressed a button. Once they have, it checks to see if thats the correct answer
    global truth_button, lie_button
    #animation - truth button
    if game_engine.mouse.collision("truth_button", display_sprite):
        truth_button.texture = game_engine.properties_object.reload_texture(f"{file_dir}/textures/truth_button.png", 60 * scale, 15 * scale)
    else:
        truth_button.texture = game_engine.properties_object.reload_texture(f"{file_dir}/textures/truth_button_2d.png", 60 * scale, 15 * scale)

    #lie button
    if game_engine.mouse.collision("lie_button", display_sprite):
        lie_button.texture = game_engine.properties_object.reload_texture(f"{file_dir}/textures/lie_button.png", 32 * scale, 15 * scale)
    else:
        lie_button.texture = game_engine.properties_object.reload_texture(f"{file_dir}/textures/lie_button_2d.png", 32 * scale, 15 * scale)
    
    #title
    if game_engine.mouse.collision("title", display):
        title.texture = game_engine.properties_object.reload_texture(f"{file_dir}/textures/title.png", 124 * scale, 15 * scale)
